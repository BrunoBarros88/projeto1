...
from .test_authors_base import AuthorsBaseTest
...
class AuthorsRegisterTest(AuthorsBaseTest):
    ...
    def get_form(self):
        return self.browser.find_element(
            By.XPATH,
            '/html/body/main/div/div/div[1]/form'
        )
    ...
    def test_empty_first_name_error_message(self):
        def callback(form):
            first_name_field = self.get_by_placeholder(form, 'Your name')
    ...
    def test_passwords_do_not_match(self):
        def callback(form):
            ...
            form = self.get_form()
            self.assertIn(
                'Password and repeat_password must be equal', form.text)
        self.form_field_test_with_callback(callback)
    ...
    def test_user_valid_data_register_successfully(self):
        ...
        self.get_by_placeholder(form, 'Your name').send_keys('First Name')
